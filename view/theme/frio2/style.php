<?php
/**
 * @file view/theme/frio2/style.php
 */

use Friendica\Core\Config;
use Friendica\Core\PConfig;

require_once 'view/theme/frio2/php/PHPColors/Color.php';
...
if ($a->module !== 'install') {
	...
	if ($uid) {
		PConfig::load($uid, 'frio2');

		// Load the profile owners pconfig.
		$scheme           = PConfig::get($uid, 'frio2', 'scheme', PConfig::get($uid, 'frio2', 'schema'));
		$nav_bg           = PConfig::get($uid, 'frio2', 'nav_bg');
		$nav_icon_color   = PConfig::get($uid, 'frio2', 'nav_icon_color');
		$link_color       = PConfig::get($uid, 'frio2', 'link_color');
		$background_color = PConfig::get($uid, 'frio2', 'background_color');
		$contentbg_transp = PConfig::get($uid, 'frio2', 'contentbg_transp');
		$background_image = PConfig::get($uid, 'frio2', 'background_image');
		$bg_image_option  = PConfig::get($uid, 'frio2', 'bg_image_option');
		$modified         = PConfig::get($uid, 'frio2', 'css_modified');

		// There is maybe the case that the user did never modify the theme settings.
		// In this case we store the present time.
		if (empty($modified)) {
			PConfig::set($uid, 'frio2', 'css_modified', time());
		}
	} else {
		Config::load('frio2');

		// Load frio2s system config.
		$scheme           = Config::get('frio2', 'scheme', Config::get('frio2', 'schema'));
		$nav_bg           = Config::get('frio2', 'nav_bg');
		$nav_icon_color   = Config::get('frio2', 'nav_icon_color');
		$link_color       = Config::get('frio2', 'link_color');
		$background_color = Config::get('frio2', 'background_color');
		$contentbg_transp = Config::get('frio2', 'contentbg_transp');
		$background_image = Config::get('frio2', 'background_image');
		$bg_image_option  = Config::get('frio2', 'bg_image_option');
		$login_bg_image   = Config::get('frio2', 'login_bg_image');
		$login_bg_color   = Config::get('frio2', 'login_bg_color');
		$modified         = Config::get('frio2', 'css_modified');
...
if (($scheme) && ($scheme != '---')) {
	if (file_exists('view/theme/frio2/scheme/' . $scheme . '.php')) {
		$schemefile = 'view/theme/frio2/scheme/' . $scheme . '.php';
		require_once $schemefile;
	}
	if (file_exists('view/theme/frio2/scheme/' . $scheme . '.css')) {
		$schemecssfile = 'view/theme/frio2/scheme/' . $scheme . '.css';
	}
}
...
if (!$scheme) {
	if (file_exists('view/theme/frio2/scheme/default.php')) {
		$schemefile = 'view/theme/frio2/scheme/default.php';
		require_once $schemefile;
	}
	if (file_exists('view/theme/frio2/scheme/default.css')) {
		$schemecssfile = 'view/theme/frio2/scheme/default.css';
	}
}
...
$css_tpl = file_get_contents('view/theme/frio2/css/style.css');

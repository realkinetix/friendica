...
<link rel="stylesheet" href="{{$baseurl}}/view/theme/frio2/frameworks/jRange/jquery.range.css" type="text/css" media="screen" />
<script src="{{$baseurl}}/view/theme/quattro/jquery.tools.min.js"></script>
<script src="{{$baseurl}}/view/theme/frio2/frameworks/jRange/jquery.range.js"></script>
...
<div id="frio2_bg_image_options" style="display: none;">
...
<script type="text/javascript">
	$(document).ready(function() {
		$("#frio2_nav_bg, #frio2_nav_icon_color, #frio2_background_color, #frio2_link_color, #frio2_login_bg_color").colorpicker({format: 'hex', align: 'left'});

		// show image options when user user starts to type the address of the image
		$("#id_frio2_background_image").keyup(function(){
			var elText = $(this).val();
			if(elText.length !== 0) {
				$("#frio2_bg_image_options").show();
			} else {
				$("#frio2_bg_image_options").hide();
			}
		});

		// show the image options is there is allready an image
		if($("#id_frio2_background_image").val().length != 0) {
				$("#frio2_bg_image_options").show();
		}
		...
		$(document).on('click', '#custom-settings-title a', function() {
			var bgTransp = $('#frio2_contentbg_transp').val();
...
<div class="settings-submit-wrapper form-group pull-right">
	<button type="submit" value="{{$submit}}" class="settings-submit btn btn-primary" name="frio2-settings-submit">{{$submit}}</button>
</div>
